...
type MetricType = "bloodPressure" | "bloodGlucose" | "heartRate" | "cholesterol" | "bmi" | "oxygenSaturation";
...
const PREDICTION_MESSAGES: Record<MetricType, Record<"low" | "medium" | "high", string>> = {
  bloodPressure: {
...
function calculatePrediction(metrics: any[]): Prediction[] {
  ...
  return metrics.reduce((predictions: Prediction[], metric) => {
    ...
    predictions.push({
      id: metric.id,
      title: `${metricType.charAt(0).toUpperCase() + metricType.slice(1)} Analysis`,
      prediction,
        ...
        <Card key={prediction.id}>
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge
                variant={prediction.risk === "low" ? "secondary" : 
                prediction.risk === "medium" ? "outline" : "destructive"}
              >
